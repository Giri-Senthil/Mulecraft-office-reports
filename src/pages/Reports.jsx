...
export default function Reports() {
  ...
  async function generateReport() {
    ...
    const [attRes, wageRes] = await Promise.all([
    ...
    const reports = activeEmployees.map((emp) => {
      ...
      const days_absent = monthAtt.filter((a) => a.status === 'absent' && a.leave_type).length
      const total_hours = monthAtt.filter((a) => a.status === 'present' || a.status === 'half-day').reduce((sum, a) => sum + (Number(a.hours_worked) || 0), 0)

      const leaveDays = att.filter((a) => a.status === 'absent' && a.leave_type)
      const types = [...new Set(leaveDays.map((a) => a.leave_type))]
      const cumulative = Object.fromEntries(types.map((t) => [t, 0]))
      let lop = 0
      ...
      for (let mm = 1; mm <= m; mm++) {
        ...
        if (mm === m) {
          types.forEach((t) => {
            lop += Math.max(0, cumulative[t] - 12) - Math.max(0, before[t] - 12)
          })
        }
      }
      ...
      return { employee_id: emp.id, month: firstDay, days_present, days_absent, total_hours, overtime_hours: 0, gross_pay, net_pay, lop }
    })
  ...
  return (
    <div>
      ...
      <div className="card">
        <table>
          <thead>
            <tr><th>Month</th><th>Employee</th><th>Present</th><th>Absent</th><th>Hours</th><th>Gross</th><th>Net</th><th>LOP</th></tr>
          </thead>
          <tbody>
            {filteredRows.length === 0 && <tr><td colSpan={8} className="muted">No reports yet.</td></tr>}
            {filteredRows.map((r) => (
              <tr key={r.id}>
